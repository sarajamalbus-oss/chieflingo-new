...
const fristUncompletedLesson = unitInActiveCourse
...
export const getLessonPrecentage = cache(async () => {
    const courseProgress = await getCourseProgress();
...
const completedchllenges = lesson.challenges.filter((challenge) => challenge.completed);
const precentage = Math.round(
    completedchllenges.length / lesson.challenges.length * 100,
);
...
export const getUserSubscription = cache(async () => {
  ...
  !isNaN(data.stripeCurrentPeriodEnd.getTime()) &&
